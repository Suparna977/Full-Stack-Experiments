...
import { getAllPosts, createPost, updatePost, deletePost } from '../data/posts';
import { authorizeAction, PERMISSIONS } from '../utils/authGuard';
...
export default function PostList() {
  ...
  const loadPosts = useCallback(async () => {
    try {
      // Even a read still goes through the same verify-then-authorize
      // pattern the backend version used — proving the token is valid
      // before trusting anything derived from it.
      await authorizeAction(token, 'view');
  ...
  async function handleCreate(e) {
    ...
    try {
      await authorizeAction(token, 'create'); // throws 403 if role isn't admin
      createPost({ title: newTitle, content: newContent, author: user.username });
      setNewTitle('');
      setNewContent('');
  ...
  async function handleUpdate(e, id) {
    e.preventDefault();
    try {
      await authorizeAction(token, 'update'); // throws 403 for viewer
  ...
  async function handleDelete(id) {
    ...
    try {
      await authorizeAction(token, 'delete'); // throws 403 unless admin
      deletePost(id);
